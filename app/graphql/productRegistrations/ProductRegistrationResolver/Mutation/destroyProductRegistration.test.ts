...
describe("destroyProductRegistration", () => {
  ...
  beforeEach(async (done) => {
    unlottedSample = await generateSampleParams({ key: "DSTRY-TST-1", lotted: false, serial: null })
    productRegistration = await ProductRegistration.create({ ...unlottedSample.sampleParams })
    done()
  })
  ...
  test("should destroy the productRegistration", async () => {
    expect.assertions(1)
    const output = await destroyProductRegistration(null, { id: productRegistration.partitionKey })
    expect(output.success).toBe(true)
  })

  test("should fail the productRegistration does not exist", async () => {
    expect.assertions(1)
    const output = await destroyProductRegistration(null, { id: "some-made-up-id-or-key" })
