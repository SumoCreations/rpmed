...
describe("updateDistributor", () => {
  ...
  test("should update the distributor", async () => {
    expect.assertions(1)
    const distributorInput = { id: distributor.partitionKey, ...sampleParams, domain: "example2.com" }
  ...
  test("should fail if the domain is already in use", async () => {
    ...
    const output = await updateDistributor(null, { distributorInput: { id: distributor.partitionKey, ...sampleParams, domain: existingDomain } })
  ...
  test("should fail if the distributor does not exist", async () => {
    expect.assertions(1)
    const output = await updateDistributor(null, { distributorInput: { id: "some-made-up-key", ...sampleParams } })
    expect(output.success).toBe(false)
  })
