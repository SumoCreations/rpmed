...
export const attachImagesToSymptom = async (
  ...
  try {
    const validations = attachedImages.map(async attachedImage => await argumentSchema.validate(attachedImage, { abortEarly: false }))
  ...
  try {
    ...
    console.log(e)
    return { success: false, errors: [ErrorProductSymptomAndAttachedImageUpdateFailed] }
  }
}