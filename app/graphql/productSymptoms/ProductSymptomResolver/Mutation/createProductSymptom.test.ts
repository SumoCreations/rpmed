...
describe("createProductSymptom", () => {
  ...
  afterAll(async () => await ProductSymptom.destroy(productSymptom.partitionKey))

  test("should generate a new productSymptom model if the productSymptom is valid", async () => {
    expect.assertions(1)
    const output = await createProductSymptom(null, { productSymptomInput: { ...sampleParams } })
    expect(output.success).toBe(true)
  })

  test("should fail if the faultCode is blank", async () => {
    expect.assertions(2)
    const output = await createProductSymptom(null, { productSymptomInput: { ...sampleParams, faultCode: "" } })
  ...
  test("should fail and report multiple invalid values", async () => {
    expect.assertions(3)
    const output = await createProductSymptom(null, { productSymptomInput: { ...sampleParams, faultCode: null, name: null } })
    expect(output.success).toBe(false)
    expect(output.errors.map(e => e.path)).toContain("faultCode")
