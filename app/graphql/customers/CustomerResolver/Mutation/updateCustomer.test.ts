...
const sampleParams = { email: "update-customer-test@example.com", name: "Jim Jeffers" }
...
describe("updateCustomer", () => {
  ...
  test("should update the customer", async () => {
    expect.assertions(1)
    const customerInput = { id: customer.partitionKey, ...sampleParams, email: "update-customer-2@example.com" }
  ...
  test("should fail the email is invalid", async () => {
    expect.assertions(2)
    const customerInput = { id: customer.partitionKey, ...sampleParams, email: "update-customer-2@not-valid" }
    const output = await updateCustomer(null, { customerInput })
  ...
  test("should fail if the email is already in use", async () => {
    ...
    const output = await updateCustomer(null, { customerInput: { id: customer.partitionKey, ...sampleParams, email: existingEmail } })
    expect(output.success).toBe(false)
    expect(output.errors.map(e => e.path)).toContain("email")
  })
  ...
  test("should fail if the customer does not exist", async () => {
    expect.assertions(1)
    const output = await updateCustomer(null, { customerInput: { id: "some-made-up-key", ...sampleParams } })
    expect(output.success).toBe(false)
  })
