...
export const import: APIGatewayProxyHandler = async event => {
 ...
 */
const processModelNumbers = (auth) => new Promise(async (resolve, reject) => {
...
const addUUIDForRow = (auth, row, uuidString): Promise<boolean> => new Promise(async (resolve, reject) => {
    const body = {
        values: [[uuidString]]
    };
    const sheets = google.sheets({ version: 'v4', auth })
