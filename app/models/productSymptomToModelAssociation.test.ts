...
import { addSymptomToModelNumber, modelNumbersForSymptom, productSymptomsForModel, removeSymptomFromModelNumber } from "./productSymptomToModelAssociation"
...
describe("modelNumber", () => {
  ...
  describe("applySymptomToModelTransactions", () => {
    ...
    it("should add the model number to the symptom's model list", async () => {
      ...
      await addSymptomToModelNumber(productSymptom.partitionKey, modelNumber.partitionKey)
    ...
    it("should return all associated model numbers for a symptom", async () => {
      ...
      expect(models.map(m => m.partitionKey)).toContain(modelNumber.partitionKey)
    })
    ...
    it("should remove the model number from the symptom's model list", async () => {
      ...
      expect((await ModelNumber.find(modelNumber.partitionKey)).symptoms).toContain(productSymptom.partitionKey)
      expect((await ProductSymptom.find(productSymptom.partitionKey)).modelNumbers).toContain(modelNumber.partitionKey)
      await removeSymptomFromModelNumber(productSymptom.partitionKey, modelNumber.partitionKey)
