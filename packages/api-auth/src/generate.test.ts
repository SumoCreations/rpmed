...
import {
  AuthVerifyUserIdFunction,
  AuthVerifyPasswordFunction,
  AuthConfig,
} from './types'
...
const verifyPassword: AuthVerifyPasswordFunction = ({
  email,
  password: pwd,
}) => {
...
const verifyUser: AuthVerifyUserIdFunction = ({ userId: id }) => {
  if (userId === id) {
    return { userId, accessLevel }
  }
  return
...
describe('generate from credentials', () => {
  describe('generateTokenFromPassword', () => {
    test('should generate tokens given valid credentials', async () => {
      expect.assertions(3)
      const tokens = await generateTokenFromPassword(AUTH_CONFIG)(
        emailAddress,
        password
      )
  ...
  describe('generateTokenFromRefreshToken', () => {
    ...
    test('should generate new credentials from a valid refresh token', async () => {
      expect.assertions(3)
      const tokens = await generateTokenFromRefreshToken(AUTH_CONFIG)(
    ...
    test('should prevent replay attacks', async () => {
      expect.assertions(1)
      await generateTokenFromRefreshToken(AUTH_CONFIG)(
        await generateValidRefreshToken()
      ) // First request
      expect(
        await generateTokenFromRefreshToken(AUTH_CONFIG)(
    ...
    test('should not generate new credentials for a non existent user', async () => {
      ...
      const refresh = jwt.sign(
        ...
        {
          algorithm: SUPPORTED_ALGORITHM,
          expiresIn: REFRESH_TOKEN_LIFESPAN ?? 60,
        }
      )
