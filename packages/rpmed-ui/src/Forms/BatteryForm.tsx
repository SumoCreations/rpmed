...
export interface BatteryFormProps extends FormProps<BatteryFormValues> {
  loading?: boolean
}

export const BatteryForm: React.FC<BatteryFormProps> = ({
  ...
  useDefaultValueListener<BatteryFormValues>(defaultValues, reset)
  ...
  const handleFormSubmit = handleSubmit(async (data) => {
    ...
    if (keys.length) {
      keys.map((key) =>
        setError(camelCase(key) as keyof BatteryFormValues, {
          message: errors[key],
        })
      )
    }
