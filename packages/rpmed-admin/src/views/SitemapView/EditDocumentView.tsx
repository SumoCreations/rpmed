...
import { Document, useMakeDocumentMutation, useDocumentQuery } from 'rpmed-schema'
import { useManagedUploads } from "./useManagedUploads"

export const EditDocumentView: React.FC = ({
}) => {
  const { handleAttachedFile, handleCreateEndpoints, previews, setPreviews } = useManagedUploads()
  ...
  const { data: existingDocumentData, loading: documentLoading } = useDocumentQuery({ variables: { id } })
  const { __typename, url, ...existingDocument } = existingDocumentData?.response.document ?? {} as Document
  ...
  useEffect(() => {
    if (existingDocument.fileKey && defaultFileKey !== existingDocument.fileKey) {
