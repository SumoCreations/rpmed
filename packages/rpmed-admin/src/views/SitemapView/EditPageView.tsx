...
export const EditPageView: React.FC = ({
  ...
  const { data: existingPageData, loading: pageLoading } = usePageQuery({ variables: { id } })
  const { __typename, ...existingPage } = existingPageData?.response.page ?? {} as Page

  const [makePage] = useMakePageMutation()
  const handleSubmit: AsyncSubmitHandler<PageFormValues> = async (formData, _action) => {
    setLoading(true)
  ...
  return (
    <Layout.Layout>
      ...
      <Content>
        ...
        <Card.Flat>
          <h2>Editing Page</h2>
          <PageForm onSubmit={handleSubmit} loading={loading || pageLoading} defaultValues={{
            ...existingPage,
