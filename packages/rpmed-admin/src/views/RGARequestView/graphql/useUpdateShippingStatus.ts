import {
  ...
  RgasQuery,
  RgaStatus,
  useUpdateRgaShippingStatusMutation,
} from 'rpmed-schema'

import { keyForStatus } from './cacheHelpers'

export const useUpdateShippingStatus = () => {
  const [updateRgaStatus] = useUpdateRgaShippingStatusMutation({
    update: (cache, { data }) => {
      ...
      try {
        const { response: oldStatusResponse } = cache.readQuery({
          query: RgasDocument,
          variables: { status: RgaStatus.Assessing },
        }) as RgasQuery
      ...
      try {
        ...
        if (rgaCount) {
          const updatedData = {
            rgaCount: {
              ...rgaCount,
              [keyForStatus(RgaStatus.Assessing)]:
                (rgaCount[keyForStatus(RgaStatus.Assessing)] || 1) - 1,
