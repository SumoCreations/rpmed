...
const ResultView = styled.div`
  background: ${(p: any) => p.theme.colorBodyTextInverted};
  display: flex;
  border: 2px solid ${(p: any) => p.theme.colorButtonPrimary};
  border-top: 0;
  box-shadow: 0 0 2px ${(p: any) => p.theme.colorButtonPrimary};
...
const Field = styled.p<{
  ...
  padding: ${(p: any) => p.padding || p.theme.inputPadding};
  flex-grow: 1;
  flex-shrink: 1;
  width: auto;
  margin-bottom: auto;
...
const Label = styled.span<{ inactive: boolean }>`
  display: flex;
  margin: auto 0;
  opacity: ${(p: any) => (p.inactive ? 0.75 : 1)};
  max-width: 100%;
  overflow: hidden;
  white-space: nowrap;
...
const Overlay = styled.span`
  background: ${(p: any) => transparentize(0.8)(p.theme.colorBodyTextInverted)};
  position: fixed;
  z-index: 4;
  height: 100vh;
  width: 100vw;
